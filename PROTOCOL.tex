...
\draw[arr, blue!70!black] ([yshift=0.3cm]cv.west) -- node[lbl, above] {Telemetría 52\,B} node[lbl, below] {\tiny UART} ([yshift=0.3cm]esp.east);
\draw[arr, blue!70!black] ([yshift=0.3cm]esp.west) -- node[lbl, above] {Telemetría 52\,B} node[lbl, below] {\tiny LoRa} ([yshift=0.3cm]et.east);
...
Paquete de \textbf{52 bytes}. El ESP32 lo reenvía por LoRa \textbf{sin modificación}. No tiene bytes CMD ni LEN -- solo sync word y footer.
...
6  & 2 & \texttt{int16}  & AccelX           & m/s\textsuperscript{2} truncado \\
8  & 2 & \texttt{int16}  & AccelY           & m/s\textsuperscript{2} truncado \\
10 & 2 & \texttt{int16}  & AccelZ           & m/s\textsuperscript{2} truncado \\
12 & 2 & \texttt{int16}  & GyroX            & deg/s truncado \\
14 & 2 & \texttt{int16}  & GyroY            & deg/s truncado \\
16 & 2 & \texttt{int16}  & GyroZ            & deg/s truncado \\
18 & 2 & \texttt{int16}  & PressurePa       & Pa $\div$ 10 \\
20 & 1 & \texttt{int8}   & TemperatureC     & \textdegree C truncado \\
21 & 4 & \texttt{int32}  & Latitude         & deg $\times$ 10\textsuperscript{7} \\
25 & 4 & \texttt{int32}  & Longitude        & deg $\times$ 10\textsuperscript{7} \\
29 & 4 & \texttt{int32}  & GPSAltitude      & m $\times$ 100 \\
33 & 1 & \texttt{uint8}  & Satellites       & crudo \\
34 & 4 & \texttt{int32}  & BaroAltitude     & m $\times$ 100 \\
38 & 4 & \texttt{int32}  & BaroVelocity     & m/s $\times$ 100 \\
42 & 4 & \texttt{uint32} & FaultFlags       & máscara de bits \\
46 & 2 & \texttt{int16}  & BatteryVoltage   & V $\times$ 10 \\
48 & 1 & \texttt{uint8}  & State            & enum \\
49 & 1 & \texttt{uint8}  & RelayState       & máscara de bits \\
50 & 1 & \texttt{uint8}  & LastCommand      & enum \\
51 & 1 & \texttt{uint8}  & SyncEnd          & \texttt{0xBE} \\
\bottomrule
\caption{Paquete de telemetría (52 bytes).}
\end{longtable}
...
// Cuando se detecta, leer 52 bytes totales (incluyendo sync).
// Verificar que byte[51] == 0xBE (footer).
// Si es valido, enviar los 52 bytes por LoRa sin modificar.
...
if (uart_byte == 0xFE) {
    ...
    if (buffer[1] == 0xCA && buffer[51] == 0xBE) {
...
\midrule
Telemetría & CV $\rightarrow$ ET & 52\,B & Passthrough & Passthrough \\
...
\begin{itemize}
    \item \textbf{Telemetría:} recibe los 52 bytes por LoRa y los escribe tal cual por serial para que la interfaz de usuario los lea y decodifique.
