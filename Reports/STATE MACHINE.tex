...
% --- Nominal flow: U-shape ---
% Row 1 (top, left to right): ground states + boost
\node[groundst] at (0, 0)    (idle)  {1. Idle};
\node[groundst] at (4, 0)    (cal)   {2. Calibration};
\node[groundst] at (8, 0)    (pre)   {3. Pre-launch};
\node[flightst] at (12, 0)   (boost) {4. Boost};

% Row 2 (middle, right to left): coast through main parachute
\node[flightst] at (12, -3)  (coast) {5. Coast};
\node[flightst] at (8, -3)   (actrl) {6. Active\\Control};
\node[pyrost]   at (4, -3)   (apogee){7. Apogee\\{\scriptsize\textit{Drogue}}};
\node[pyrost]   at (0, -3)   (main)  {8. Main\\Parachute};

% Row 3 (bottom): terminal + aborts
\node[terminalst] at (0, -6)  (landed) {9. Landed};
\node[abortst]    at (4, -6)  (dabort) {11. Descent\\Abort};
\node[abortst]    at (8, -6)  (gabort) {10. Ground\\Abort};

% --- Nominal transitions (black solid) ---
\draw[arr] (idle)   -- node[lbl, above] {CMD Calibration\\or auto-start} (cal);
\draw[arr] (cal)    -- node[lbl, above] {Sensors\\calibrated} (pre);
\draw[arr] (pre)    -- node[lbl, above] {$a_Y >$ thr.\\$\times N$ samples} (boost);
\draw[arr] (boost)  -- node[lbl, right] {$a_Y <$ thr.\\$\times N$ samples} (coast);
\draw[arr] (coast)  -- node[lbl, above] {Alt $\geq$ thr.} (actrl);
\draw[arr] (actrl)  -- node[lbl, above] {Apogee\\detected} (apogee);
\draw[arr] (apogee) -- node[lbl, above] {Alt $\leq$ thr.\\or timeout} (main);
\draw[arr] (main)   -- node[lbl, right] {Landing\\detected} (landed);

% --- Ground abort (red solid, from each ground state straight down) ---
\draw[faultarr] (pre)  -- node[lbl, right] {Fault} (gabort);
\draw[faultarr] (cal.south)  -- ++(0,-0.8) -| node[lbl, below, pos=0.7] {Fault} (gabort.north west);
\draw[faultarr] (idle.south) -- ++(0,-0.8) -| node[lbl, below, pos=0.85] {Fault} ([xshift=-3mm]gabort.north west);

% --- Ground abort reset (dashed, routed around the left side) ---
\draw[cmd] (gabort.west) -- ++(-0.6, 0) |- node[lbl, left, pos=0.7] {CMD\\Reset} (idle.south);

% --- Descent abort (red solid, from Apogee straight down) ---
\draw[faultarr] (apogee) -- node[lbl, right] {Sensor\\fault} (dabort);

% --- Descent abort to landed (black solid) ---
\draw[arr] (dabort) -- node[lbl, above] {Landing\\detected} (landed);
