...
\begin{table}[H]
    ...
    \hline
         \textbf{N} & \textbf{L} & \textbf{D\textsubscript{ext}} & \textbf{P\textsubscript{0}} & \textbf{t\textsubscript{b}} & \textbf{I\textsubscript{t}} & \textbf{F\textsubscript{max}} \\
         \hline
...
The flight state machine comprises 12~states organized into four functional groups: ground operations, powered and ballistic flight, recovery, and terminal states. Figure~\ref{fig:state_machine} illustrates the complete state diagram with all transition conditions.
...
\node[groundst]   at (0, 0)     (idle)   {Idle};
\node[groundst]   at (0, -2.4)  (cal)    {Calibration};
\node[groundst]   at (0, -4.8)  (pre)    {Pre-launch};
\node[flightst]   at (0, -7.2)  (boost)  {Boost};
\node[flightst]   at (0, -9.6)  (coast)  {Coast};
\node[flightst]   at (0, -12.0) (actrl)  {Active\\Control};
\node[pyrost]     at (0, -14.4) (apogee) {Apogee\\{\scriptsize\textit{Drogue}}};
\node[pyrost]     at (0, -16.8) (main)   {Main\\Parachute};
\node[terminalst] at (0, -19.2) (landed) {Landed};

% --- Abort states (right column) ---
\node[abortst] at (6, -4.8)  (gabort) {Ground\\Abort};
\node[abortst] at (6, -9.6)  (aabort) {Coast/Control\\Abort};
\node[abortst] at (6, -14.4) (dabort) {Descent\\Abort};
...
\draw[faultarr] (apogee.east) -- node[lbl, above] {Fault} (dabort.west);

% --- Coast/Control abort (red solid, diagonal from Coast and Active Control) ---
\draw[faultarr] (coast.east) -- node[lbl, above, sloped] {Fault} (aabort.west);
\draw[faultarr] (actrl.east) -- node[lbl, above, sloped] {Fault} (aabort.west);

% --- Coast/Control abort to apogee (black solid, diagonal) ---
\draw[arr] (aabort.south) -- node[lbl, above, sloped] {Apogee\\detected} (apogee.east);
...
The \textbf{Coast/Control Abort} state is entered when a fault is detected during the unpowered ascent phase (Coast or Active Control). On entry, the airbrake servo is commanded to full deployment to maximise aerodynamic drag and rapidly dissipate the remaining kinetic energy. The vehicle continues its ballistic arc under full braking until apogee is reached. \req{EuRoC-LV-RQT-0670}
...
Landed / Ground Abort / Descent Abort / Coast/Control Abort & Low power (idle) & Stopped \\
\bottomrule
\end{tabularx}
\end{table}
...
Anomalies detected in telemetry or control loops force an immediate transition to the respective Abort state. A \textbf{Ground Abort} halts the launch sequence. A \textbf{Coast/Control Abort} commands full airbrake deployment to maximise drag and dissipate kinetic energy during unpowered ascent; the vehicle continues under full braking until apogee. A \textbf{Descent Abort} skips main parachute deployment, allowing the vehicle to descend under the drogue parachute alone at a higher descent rate until landing detection.
